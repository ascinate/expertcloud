...
export default function MultiStep (props) {
  ...
  const renderNav = (show) =>
    show && (
      <div>
        <button className='back-rgs btn'
          style={buttonsState.showPreviousBtn ? props.prevStyle : { display: 'block' }}
          onClick={previous}
        >
        <FaAngleLeft/>
          Back
        ...
        <button className='conti-new btn'
  ...
  return (
    <div>
      ...
      {inactiveComponentClassName
        ? props.steps.map((step, FormWizard) => {
            const className = FormWizard === compState ? activeComponentClassName : inactiveComponentClassName
            return (<div className={className} key={FormWizard}>{step.component}</div>)
          })
